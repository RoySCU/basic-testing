...
on_data(const uint8_t* rawdata, uint32_t data_size, void* userdata)
{
  ...
  ndn_name_t result;
  ndn_name_init(&result);
  tlv_parse_data(rawdata,data_size, 1 ,TLV_DATAARG_NAME_PTR, &result);
  printf("on_data: data name:");ndn_name_print(&result);putchar('\n');
  //save_file(data_buf);
}
...
//   ndn_forwarder_register_prefix(encoder.output_value, encoder.offset, on_interest, NULL);
  ...
  //ps_subscribe_to(NDN_SD_TEMP, id, 2, 300000, on_data);
